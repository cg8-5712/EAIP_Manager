...
from src.builder.aipkg_format import (
    AIPKGHeader,
    FileEntry,
    PackageIndex,
    COMPRESSION_GZIP,
...
class AIPKGBuilder:
    ...
    def create_package(
        ...
        try:
            ...
            # 3. 迭代构建索引直到长度稳定
            # 由于文件偏移值会影响 JSON 长度，需要迭代直到收敛
            logger.info("Building index with correct offsets...")
            if progress_callback:
                progress_callback(0, 100, "构建索引...")

            index_iv = generate_iv()
            logger.debug(f"[BUILD] Generated index IV: {index_iv.hex()} (length: {len(index_iv)})")
            ...
            while iteration < max_iterations:
                ...
                logger.debug(f"[BUILD] Iteration {iteration}: index_length = {index_length}")

                # 检查是否收敛
                if index_length == prev_index_length:
                    logger.info(f"[BUILD] Index converged after {iteration} iteration(s)")
                    break
                ...
                for entry in file_entries:
                    if iteration == 1:
                        ...
                        # 后续迭代：调整索引长度变化导致的偏移差值
                        delta = index_length - prev_index_length
                        entry.offset += delta

                prev_index_length = index_length

            if iteration >= max_iterations:
                logger.warning(f"[BUILD] Index did not converge after {max_iterations} iterations")

            logger.info(f"[BUILD] Final encrypted index: {len(index_json)} -> {len(encrypted_index)} bytes")
            ...
            with open(temp_output, 'w+b') as f:  # 使用 w+b 模式，允许读写
                ...
                logger.debug(f"[BUILD] Wrote placeholder header: {HEADER_SIZE} bytes")

                # 写入加密索引
                f.write(encrypted_index)
                logger.debug(f"[BUILD] Wrote encrypted index: {len(encrypted_index)} bytes at offset {HEADER_SIZE}")
                ...
                for idx, encrypted_data in enumerate(encrypted_blocks):
                    ...
                    total_data_written += len(encrypted_data)

                logger.info(f"[BUILD] Wrote {len(encrypted_blocks)} data blocks, total {total_data_written} bytes")
                ...
                # 获取当前文件大小（flush 前）
                current_pos = f.tell()
                logger.debug(f"[BUILD] Current file position before flush: {current_pos}")

                f.flush()  # 确保所有数据都写入缓冲区
                logger.debug(f"[BUILD] Flushed write buffer")

                # 计算从 HEADER_SIZE 开始的所有内容的哈希
                ...
                logger.info(f"[BUILD] Computed file hash over {bytes_hashed} bytes (from offset {HEADER_SIZE})")
                logger.info(f"[BUILD] File hash (hex): {file_hash.hex()}")

                # SHA256 是 32 字节，但 Header 的 file_hash 字段是 64 字节，需要填充
                file_hash_padded = file_hash + b'\x00' * 32
                logger.debug(f"[BUILD] Padded file hash to 64 bytes: {file_hash_padded.hex()}")
                ...
                logger.debug(f"[BUILD] Header fields:")
                logger.debug(f"  - index_offset: {index_offset}")
                logger.debug(f"  - index_length: {index_length}")
                logger.debug(f"  - index_iv (hex): {index_iv.hex()}")
                logger.debug(f"  - index_iv (length): {len(index_iv)}")
                logger.debug(f"  - master_salt (hex): {self.master_salt.hex()}")
                logger.debug(f"  - total_files: {len(file_entries)}")
                logger.debug(f"  - file_hash: {file_hash_padded.hex()}")

                f.seek(0)
                f.write(final_header.to_bytes())
                f.flush()  # 确保 Header 完全写入磁盘
                logger.debug(f"[BUILD] Wrote final header and flushed")
