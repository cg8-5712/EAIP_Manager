...
class AIPKGHeader:
    ...
    Attributes:
        ...
        index_iv: 索引加密 IV (12 bytes, AES-GCM 标准)
        master_salt: 主密钥派生盐值 (32 bytes)
        file_hash: 文件哈希 (64 bytes, SHA256 + padding)
    ...
    def to_bytes(self) -> bytes:
        ...
        # 打包数据
        data = struct.pack(
            '<4sHHQQ12s32s64sQQQII128s220s',  # IV 改为 12 字节，reserved 改为 220 字节
            self.magic,
            self.version_major,
            self.version_minor,
            self.index_offset,
